...
obj_to_san_d = lambda obj: {
    attr: getattr(obj, attr)
    for attr in dir(obj)
    if not attr.startswith("__")
    and type(getattr(obj, attr)) in normal_types
...
class Compute(object):
    ...
    def set_node(self):
        ...
        if (
            self.provider_dict["provider"]["name"] == "GCE"
            and "client_id" in self.provider_dict["auth"]
            and "user_id" not in self.provider_dict["auth"]
        ):
            self.provider_dict["auth"].update(
                {
                    "user_id": "",
                }
            )  # "key": ""})
        ...
        if self.provider_cls.type.startswith("azure"):
            ...
            if "create_with" in self.provider_dict:
                ...
                if "network" in self.provider_dict:
                    ...
                    subnet = AzureSubnet(
                        id="eth0",
                        name=self.provider_dict["network"]["name"],
                        extra=None,
                    )
                    print(
                        "list_nics = {};".format(
                    ...
                    if subnet in self.provider_cls.ex_list_nics(
                        ...
                        nic = self.provider_cls.ex_create_network_interface(
                            self.provider_dict["network"]["name"],
                            subnet,
                            self.node_specs["ex_resource_group"],
                        )
                        print("nic =", nic)
            ...
            if "extras" not in self.strategy.strategy_dict["node"]:
                self.strategy.strategy_dict["node"]["extras"] = dict(
                    options=(self.provider_dict,)
                )

            hardware, image, extras = list(
                map(
                    (
                        lambda typ: next(
                            dict(key=self.provider_dict["provider"]["key"], **spec)
                            for spec in self.strategy.strategy_dict["node"][typ][
                                "options"
                            ]
                            for key in ("region", "name")
                            if spec["provider"][key]
                            == self.provider_dict["provider"][key]
                        )
                    ),
            ...
            if "extra" not in nodesize_kwargs:
                ...
                nodesize_kwargs.update(
                    **{"extra": {"provider": hardware["provider"]["visor"]}}
                )
        ...
        if "security_group" in self.provider_dict:
            self.node_specs.update(
                {"ex_securitygroup": self.provider_dict["security_group"]}
            )
    ...
    def setup_keypair(self):
        try:
            ...
            logger.warn(
                "`import_key_pair_from_file` not implemented for {}".format(
                    self.provider_dict["provider"]["name"]
                )
            )
            pass  # DW about it
    ...
    def attempt_provision(
        self, create_or_deploy="create", prefer_provider=None, prefer_image=None
    ):
        ...
        if prefer_provider:
            ...
            """
        for i in range(
            len(self.strategy.strategy_dict["provider"]["options"])
        ):  # Threshold
            logger.info(
                "Attempting to create node {node_name!r} on: {provider!r}".format(
                    node_name=self.node_name,
                    provider=self.provider_dict["provider"]["name"],
                )
            )
    ...
    def provision(self, create_or_deploy):
        ...
        try:
            self.node = getattr(self, "{0}_node".format(create_or_deploy))(
            ...
            if e.message != error_message:
                raise
            logger.info(
                "{error_message}, so running `create_node` instead.".format(
